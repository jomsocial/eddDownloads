...
if( ! current_user_can( 'edit_shop_payments' ) && $user_id != get_current_user_id() ) {
	...
	?><p class="edd_sl_no_keys"><?php _e( 'There are no license keys for this purchase', 'edd_downloads' ); ?></p><?php
	...
	<table id="edd_downloads_license_keys" class="edd_downloads_table">
		...
		<thead>
			...
			<tr class="edd_downloads_license_header">
				...
				<th class="uk-hidden-small"><?php _e( 'Duration', 'edd_downloads' ); ?></th>

				<th class="uk-hidden-small"><?php _e( 'Support Domain', 'edd_downloads' ); ?></th>

				<th class="uk-hidden-small"><?php _e( 'Download', 'edd_downloads' ); ?></th>

				<th class="uk-hidden-small"><?php _e( 'Renew / Upgrade', 'edd_downloads' ); ?></th>                
						...
						if(!empty($product)){
							...
							<tr class="edd_downloads_license_row">
								...
								<td>
                                    ...
                                    <div class="edd_downloads_file_name">
                                        ...
                                        <?php echo $cart_item['name']; 
                                              ...
                                              echo '<div class="uk-text-muted edd_license_filename">' . $file['name'] . '.zip' . '</div>'; 
									...
									<div class="edd_downloads_license_number">
										<?php _e( 'License No: ', 'edd_downloads' ); ?>
										<span><?php echo $licensing->get_license_key( $licenses[$cart_row]->ID ); ?></span>
									</div>

								</td>

								<td class="uk-hidden-small"><strong><?php echo edd_downloads_get_product_duration($cart_item); ?></strong></td>

								<td class="uk-hidden-small">

									<?php $disabled = isset($products_domains[$product->ID]) && !empty($products_domains[$product->ID]) ? 'disabled' :'';?>

									<?php $disabled = isset($products_domains[$product->ID."_changes"]) && $products_domains[$product->ID."_changes"] > 1 ? 'disabled' :'';?>

									<form action="<?php echo site_url($Path); ?>" method="post" class="edd_downloads_domains" id="edd_downloads_domains_domain_form_<?php echo absint( $licenses[$cart_row]->ID ) ?>">
										<small><?php _e('No www or http://','edd_downloads'); ?></small>

										<input id="edd_downloads_domains_domain_<?php echo absint($licenses[$cart_row]->ID); ?>" class="ks input" <?php echo $disabled; ?> type="text" name="edd_downloads_domains_domain" value="<?php echo (isset($products_domains[$product->ID])) ? esc_url($products_domains[$product->ID]) : '' ; ?>" autocomplete="off" />
										
										<?php if($disabled == 'disabled'){ ?>

											<button style="submit" onClick="return check_domain(<?php echo absint($licenses[$cart_row]->ID); ?>);" class="button edd-submit" <?php echo $disabled; ?>><?php echo _e('Save','edd_downloads'); ?></button>

										<?php }else{ ?>

											<button style="submit" onClick="return check_domain(<?php echo absint($licenses[$cart_row]->ID); ?>);" class="button blue edd-submit"><?php echo _e('Save','edd_downloads'); ?></button><br/>
								...
								<td class="uk-text-center"><?php
									...
									if(isset($products_domains[$product->ID]) && !empty($products_domains[$product->ID]) && !empty($download_url) && 0 === $expired_license){
										...
										?><button style="submit" onClick="javascript:alert('<?php echo _e('Your license has expired. Please renew your license to be able to download again.','edd_downloads'); ?>'); return false;" class="button edd-submit"><?php echo _e('Download','edd_downloads'); ?></button><?php
								...
								<td>
									...
									if( 'expired' == edd_software_licensing()->get_license_status( $licenses[$cart_row]->ID ) ) {
										...
										<div class="edd_purchase_submit_wrapper">
											...
											$edd_options[$post_meta]					= array( 'download_id' => $post_meta, 'payment_id' => $payment_id, 'license_id' => $licenses[$cart_row]->ID, 'product_id' => $product->ID, 'expired' => 1 );

											

											//print_r($edd_options);

											echo edd_get_purchase_link( array( 'download_id' => $post_meta, 'payment_id' => $payment_id, 'license_id' => $licenses[$cart_row]->ID, 'product_id' => $product->ID, 'expired' => 1 ) );
											echo _e( 'Product ' . $cart_item['name'] . ' expired ' ,'edd_downloads' );
